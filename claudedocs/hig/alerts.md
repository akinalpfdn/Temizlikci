...
- macOS alerts can add a suppression [toggles#Checkboxes] and a [buttons#Help-buttons].
...
Avoid explaining alert buttons. If your alert text and button titles are clear, you don’t need to explain what the buttons do. In rare cases where you need to provide guidance on choosing a button, use a term like choose to account for people’s current device and interaction method, and refer to a button using its exact title without quotes. For guidance, see [alerts#Buttons].
...
- Include a Help button that opens your help documentation (see [buttons#Help-buttons]).
...
If someone moves a window without dismissing its alert, the alert remains anchored to the window. If your app is running in a Full Space, the system displays the alert centered in the wearer’s [spatial-layout#Field-of-view].
...
[alert(_:isPresented:actions:)-1bkka] — SwiftUI
